...
#' Plot the age breakdown of a computed CHAMPS table
#'
...
#' @import ggfittext
#' @export
plot_agebreakdown <- function(x, legend_location = "none", plot_type = "pie", 
                              include_pie_numbers = FALSE, include_bar_text = TRUE,
                              waffle_cols = 12) {
  
  check_champs_object(x)
  
  d1 <- dplyr::ungroup(x$age_breakdown)
  
  piecolors <- c("#ffb82b", "#9b9b9b", "#ee6f34", "#3966b9", "#4d90cc")
  
  if (!any(plot_type %in% c("pie", "bar", "tree", "waffle"))) stop("Expecting one of 'pie', 'bar', 'treemap' or 'waffle'")
  
  if (plot_type == "pie") {
    p_out <- d1  %>%
      ggplot2::ggplot(ggplot2::aes(x = "", y = n, fill = case_type_desc)) +
    ...
    if (include_pie_numbers) {
      p_out <- p_out + 
        ggplot2::geom_text(ggplot2::aes(label = n), position = ggplot2::position_fill(vjust = 0.5)) 
    }
    
  } else if (plot_type == "bar" ) {
    # https://github.com/wilkox/ggfittext
   p_out <- d1  %>%
      dplyr::mutate(xuse = forcats::fct_reorder(case_type_desc, n, .desc = TRUE)) %>%
      ggplot2::ggplot(ggplot2::aes(x = xuse, 
                                   y = n, fill = xuse, label = xuse)) +
      ggplot2::geom_col() +
      ggplot2::scale_fill_manual(values = piecolors) +
      ggplot2::guides(fill = ggplot2::guide_legend(nrow=2,byrow=TRUE)) +
      ggplot2::labs(fill = "") +
    ...
    #   ggplot2::ggplot(ggplot2::aes(x, y, fill = case_type_desc)) + 
